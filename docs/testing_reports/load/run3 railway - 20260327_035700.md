...
The `railway run -p -e -s -- psql $DATABASE_PUBLIC_URL -c TRUNCATE ...` + `seed.py` reset before each run ensures no leftover meta-field state carries over between test runs and the seed data count is consistent.

Note: the DB reset was already present in run 2 — it was introduced in commit `edc8b95` the day before run 2 ran. The 772 HTTP 401 errors on `POST /auth/refresh` in run 2 were **not** caused by stale state from a prior run, and the DB reset was not what fixed them. The real cause was non-atomic token rotation interrupted by DB stress: `RefreshAccessToken` deletes the old token first, then issues a new one (`internal/auth/jwt.go:97–100`). Under DB overload, the delete succeeded but the new token issuance failed — leaving users permanently unable to refresh. Run 3 had 0 DB errors throughout (due to points 1 and 2 above), so the rotation was never interrupted.
