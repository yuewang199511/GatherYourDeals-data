...
| 2026-03-29 | — | Replaced VNet + private PostgreSQL with public B1ms + Azure-services firewall | VNet caused persistent DNS resolution failures (private DNS zones unresolvable from Container Apps); public access + credential protection matches Railway's security posture at lower complexity |
| 2026-03-29 | — | Replaced native pgBouncer with separate pgBouncer Container App (internal ingress) | Burstable tier does not support native pgBouncer; separate Container App with min 1 replica mirrors Railway's pgBouncer service topology |
| 2026-03-29 | — | Added CI goose migration step as gydadmin before app start | Enables future split to restricted gydapp runtime user without app code changes; ensures DDL runs as admin, app runtime as DML-only user |
